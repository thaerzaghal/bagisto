...
- **Central-host exception rendering safety — DONE (TASK-MVP-008, RISK_REGISTER.md R68).** A real, broader-than-signup production defect found as a byproduct of R67: `Webkul\Core\Exceptions\Handler` forces any HTTP exception status outside `{401,403,404,503}` (405/419/422/429/500) into a themed error view that unconditionally queries tenant-only tables - crashing with a raw 500 for any request where tenancy was never initialized (any central Platform route, or a route matching no pattern for its HTTP verb). Fixed via a new, Platform-owned `Platform\Tenancy\Exceptions\CentralSafeExceptionHandler`, decorating whatever handler Webkul bound via `Container::extend()` - activates only under `APP_DEBUG=false` + `!tenancy()->initialized` + no self-rendering exception (excluding R57/R58's own `TenantNotReadyHttpException` by design), otherwise delegates unchanged. Zero `packages/Webkul/*` changes. R51/R53/R54/R57/R58 all confirmed unaffected. R62 (a separate, pre-existing, unknown-domain eager-controller-construction crash) remains explicitly open and untouched.
