...
- **Public signup abuse protection — LOCALLY COMPLETE, awaiting real Cloudflare credentials (TASK-MVP-006, RISK_REGISTER.md R66).** `Platform\Signup\Services\TurnstileVerifier` gates `POST /join` before any provisioning side effect, fails closed when enabled, `TURNSTILE_ENABLED=false` by default (correct for the current invited-only pilot). `POST /join`'s throttle tightened to `throttle:3,1`. Email-ownership verification was evaluated and explicitly deferred (DECISION_LOG.md C77) - Turnstile + throttle are judged sufficient for the first Public MVP. **Real Cloudflare Turnstile site/secret keys are not yet configured in production** - `TURNSTILE_ENABLED` remains `false` live until the operator configures them and a real-browser proof is completed.
