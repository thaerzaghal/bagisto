...
**Status update (TASK-ARCH-016)**: the `Subscription` domain (`packages/Platform/Subscriptions`, `Platform\Subscriptions\Services\SubscriptionLifecycle`) now exists and is real, tested code — see [subscriptions.md](subscriptions.md) — but it does **not** yet depend on any `BillingProvider` interface, because none exists yet; `SubscriptionLifecycle`'s manual transitions (`start()`, `cancelImmediately()`, etc.) are called directly by Platform Admin today, with zero payment-provider involvement anywhere in the call chain. The diagram above remains the intended Phase 11 shape - when a provider is selected, `BillingProvider`'s concrete implementation (webhooks, payment-method state) will call into `SubscriptionLifecycle`'s existing transition methods rather than duplicating them, not the other way around. This keeps Phase 11 additive to the already-built lifecycle domain, not a rework of it.
