...
No Subscription/Billing/Invoices/Usage-history UI exists, because those domains don't exist yet (no subscriptions, no billing provider, no usage tracking — TASK-ARCH-008's own scope boundary). Their future conceptual location: a `SaaS` menu section (already established by this task) gains sibling children (`saas.subscription`, `saas.billing`, `saas.usage`) once Phase 10/11 build the underlying domains — no structural rework needed, since the menu/ACL merge mechanism this task established already supports adding more entries the same way. No enforcement of plan limits exists either (`products.limit` is displayed, never checked against Bagisto's actual product count) — a later, separate task.
