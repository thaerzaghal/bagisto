...
**Updated (TASK-ARCH-016)**: real subscription lifecycle state (status/trial/period/cancellation intent) is now shown on this same page - see docs/architecture/subscriptions.md "Tenant-visible subscription status". `products.limit` enforcement is real since TASK-ARCH-012 (no longer "displayed, never checked" - see docs/architecture/feature-limits.md).

**Updated (TASK-ARCH-019)**: a real checkout flow now exists, owned entirely by `Platform\Billing` - see docs/architecture/billing.md "Checkout flow" and "Tenant-facing entry point". Deliberately a SEPARATE package's routes/controller/views (`Platform\Billing\Http\Controllers\Tenant\CheckoutController`, `admin.saas.checkout.*`), not folded into `MyPlanController`/`Platform\Plans` - `Platform\Plans` must never depend on `Platform\Billing` (the dependency direction runs the other way). The existing "My Plan" page gained one plain `route()`-based link ("Upgrade Plan") to the new checkout page - a Blade-level connection only, zero PHP/controller-level import from `Platform\Plans` into `Platform\Billing`. New ACL key `saas.checkout` (`packages/Platform/Billing/src/Config/acl.php`), merged the same way `Platform\Plans\Providers\PlansServiceProvider` merges its own. No invoices/payment-method-vault UI exists still - out of this task's scope too (see billing.md's own out-of-scope list).
