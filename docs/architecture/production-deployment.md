...
**Secret disclosure - a real defect found and fixed (TASK-OPS-MONITORING-001A finding 1), then hardened again (TASK-OPS-MONITORING-001B finding 2).** Neither the notification email, the persisted state file, nor this command's own console output ever includes a raw caught-exception message - `ProductionMonitorRunner::describe()` includes only the exception's class name, never `getMessage()`, since a caught exception can originate anywhere in the application and is fundamentally unbounded, unreviewed input. Separately, `ReadinessCheckResult::detail` text (first-party, reviewed code, but not assumed safe merely because of that - two of `platform:production:check`'s own existing checks, `checkRedis()`/`checkThemeStaticAssets()`, already embed a raw exception message into their own `detail` string) is **structurally never forwarded to a real (new/changed/reminder/recovered) notification at all** - `ProductionMonitorRunner::processOne()` builds every such notification from only the check label, reason, status, and timestamp, and `ProductionAlertMail::body()` renders no per-notification detail for them, only a fixed static footer line pointing the operator at running `platform:production:check` on the server itself. This replaced an earlier design where `detail` text passed through `Platform\Tenancy\Support\NotificationRedactor`, a generic pattern-based redactor, before reaching the notification - a real, reproduced review finding showed that pattern list could be bypassed by shapes it did not recognize (a JSON-shaped `{"password":"..."}`, a space-containing `password="..."` value, an `Authorization: Basic ...` header). `NotificationRedactor` still exists and is still applied (to `describe()`'s own already-safe, class-name-only output) as defense-in-depth, but is no longer the primary guarantee for anything. The one exception is the synthetic `--test-notification` message, whose `detail` is a single hardcoded string literal, safe by construction rather than by inspection.
...
`<MONITOR_STATE_PATH>/state.json` - one JSON object, `checks` (per-check status/onset-time/last-successfully-notified status+timestamp) and `meta` (last run time, last monitor-level error if any, last delivery-skip reason if any). Contains no secrets, no tenant/customer data, no stack traces, and (TASK-OPS-MONITORING-001B fix 2) no free-form check `detail` text at all - only a check label, status, and timestamps.

**State validation and corruption handling - a real defect found and fixed (TASK-OPS-MONITORING-001B finding 3).** Reading this file no longer only checks PHP types - each entry's `status`/`last_notified_status` must be a real check status, `since`/`last_notified_at` must actually parse as a timestamp, and `last_notified_status`/`last_notified_at` must be consistently both-set or both-null. An entry failing any of these is dropped (never crashes the run, never falsely resurrects as healthy) while every other, unrelated valid entry is preserved untouched; the number of entries dropped is surfaced on the console as a plain count, never as raw file content. Separately, a state file that **exists but is not valid JSON in the expected shape at all** is now treated as a monitor failure (alerted like any other) and is **never silently overwritten with a fresh baseline** by that run's own write - a real, reproduced defect in the original design, where the very next run would have permanently destroyed a corrupt-but-possibly-recoverable file. A genuinely missing file (first-ever run) remains a safe, empty baseline.
...
**`--dry-run` and `--test-notification` are mutually exclusive - never combine them (TASK-OPS-MONITORING-001B finding 1).** They express opposite intents (preview nothing-sent vs. deliberately send one real email); passing both is rejected outright, before the lock file or any other filesystem operation, with a non-zero exit and a clear diagnostic - never silently prioritizing one flag over the other.
