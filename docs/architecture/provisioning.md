...
## Channel hostname correctness — IMPLEMENTED (TASK-MVP-003)

Step 11 below ("Bagisto's seeded default channel needs its `hostname` set to the tenant's actual domain, which is tenant-specific and can't come from the generic seeder") was Phase 0 speculation too - now real. `Webkul\Installer\Database\Seeders\Core\ChannelTableSeeder` always seeds the tenant's one default channel (`id=1`) with `hostname = config('app.url')` - the CENTRAL app's own URL, never the tenant's own domain (that seeder has no per-tenant concept at all). `TenantProvisioner::ensureChannelHostnameCorrect()` (a new, unconditional pipeline step, no optional parameter needed - unlike owner-admin identity, this needs no external input) corrects it to the tenant's real primary domain, resolved from the central `Tenant`/`Domain` data (never guessed from the tenant id).

**Format, verified from Bagisto source, not assumed**: a bare hostname (e.g. `mystore.platform.test`), never a scheme-prefixed URL. Confirmed three ways: the Admin "Host Name" field validates with a plain `unique:channels,hostname` rule (no `url:` rule); `Webkul\Core\Core::getCurrentChannel()`'s own hostname lookup explicitly checks the bare form as one of three candidate values it queries; and both real consumers (`Webkul\Shop\Http\Controllers\SitemapController`, `Webkul\Sitemap\Jobs\ProcessSitemap` - the only places `channels.hostname` is actually read, both via `sitemap.xml`/`robots.txt` generation) already normalize a schemeless value to `https://` themselves before using it as a URL base. Writing a scheme here would hardcode a decision (this environment currently serves tenants over plain `http://` locally) this step has no business making - deferred correctly to read time, and to TASK-MVP-004's eventual real production scheme decision.

**Scope discipline**: targets `channels.id = 1` (the one channel every tenant has) and only the `hostname` column - no other channel field (theme, currency, locale, root category) is ever read or written, so a merchant's own configuration is never disturbed by this step, on first provisioning or on repeat.

**Existing-tenant repair**: `php artisan platform:tenants:repair-channel-hostname {--tenant=*}` (`Platform\Tenancy\Console\Commands\RepairChannelHostname`), mirroring `platform:tenants:migrate-pending`'s exact shape - a narrowly-scoped, single-purpose command, deliberately not folded into that broader one. Unlike that command, this one only ever processes `Ready` tenants (skipping, not erroring on, any other status) - a Pending/Provisioning/Failed tenant has no reliable guarantee its `channels` table exists yet, and `provision()` already applies this same step correctly once such a tenant successfully reaches Ready. Never reprovisions, reseeds, or migrates anything.
